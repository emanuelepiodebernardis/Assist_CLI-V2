...
class ExplainerAgent(BaseAgent):
    ...
    def self_check(
        ...
        prompt = PromptBuilder.build_explain_self_check_prompt(
            draft=draft,
            task=task,
            skills=skills,
        )
        ...
        return ValidationReport(
            ...
            quality_score=_clamp_score(parsed.get("quality_score", 0.0)),
            clarity_score=_clamp_score(parsed.get("clarity_score", 0.0)),
            issues=issues,
            actions=[
                str(action)
    ...
    def correct(
        ...
        prompt = PromptBuilder.build_explain_correction_prompt(
            draft=draft,
            report=report,
            task=task,
            skills=skills,
