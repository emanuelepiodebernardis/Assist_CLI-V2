...
- auto-discovery of `<name>.test.ts` / `<name>.spec.ts`
- **vitest** execution in an isolated Node sandbox (~0.7s per run)
- boundary tests generated by the fast model
- property-based testing with **fast-check** (`numRuns` enforced via
  `configureGlobal`)
- same evidence-based judge, PR comment, certificate and telemetry as the
  Python flow — evidence is language-agnostic by design

One-time setup: `mkdir -p ~/.assist/ts-template && cd ~/.assist/ts-template
&& npm init -y && npm install vitest fast-check typescript`. Details and
current limits (single-file mutation needs a StrykerJS project, no TS fix
loop yet) in [`docs/typescript.md`](docs/typescript.md).

## Benchmark results

Numbers from `benchmark/run_benchmark.py`, run against a corpus of 20
realistic AI-style bugs across 8 categories and 12 domains (pagination,
retry logic, rate limiting, permissions, dates, carts, scoring...) (off-by-one, wrong boolean logic, missing early
...
406 tests, no flaky tests, no API calls required (mock LLM clients and a
...
- [`docs/typescript.md`](docs/typescript.md) — TypeScript verification
- [`SECURITY.md`](SECURITY.md) — sandbox isolation model (process /
  Docker / TS) and what the LLM sees
- [`TECH_DEBT.md`](TECH_DEBT.md) — known limitations, severity, workarounds

## Tech stack

Python 3.10+, Typer, Pydantic v2, Rich, PyYAML, pytest, Hypothesis,
coverage/pytest-cov. LLM: Anthropic SDK or any OpenAI-compatible
endpoint (stdlib client) — or none at all. TypeScript flow: Node 18+,
vitest, fast-check.
